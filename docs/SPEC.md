...
- The GitHub App must be installed on the meta repo **and every child repo** it should be able to open PRs on. A submodule outside the installation is **never checked out**: the runner inits only the paths `/runner/token` reports as reachable (§10.2), the push loop skips the unpopulated ones, and the gateway names them in the completion message ("Skipped (the Heimdall GitHub App is not installed on them): …").
- Partial coverage is a first-class case, not an error. A meta repo may legitimately span orgs the App will never be installed on (vendored infra, another team's services), and a run must still do the work it _can_ do. This is why checkout is `submodules: false` + selective init rather than `submodules: recursive`: recursive clones every `.gitmodules` entry, so a single unreachable private child fails the checkout step and kills the run before Claude starts — turning a partial-coverage repo into a total outage.
...
- Mints an installation token **scoped via the `repositories` parameter to exactly `[root, ...submodules]`** — never the whole installation — and returns `{ token, repos, submodule_paths, expires_at }`. `submodule_paths` is the working-tree path of each *covered* submodule, and it is what the runner inits. Paths rather than URLs on purpose: the gateway already resolves relative/ssh `.gitmodules` URLs in TypeScript (`resolveSubmoduleUrl`), and a bash reimplementation in the runner would be a second, divergent parser.
...
1. **Checkout**: `submodules: false`, then a separate init step running `git submodule update --init -- <submodule_paths>` with the §10.2 token, authenticated via `git config url."https://x-access-token:<token>@github.com/".insteadOf` for both the `https://` and `git@github.com:` forms. Empty `submodule_paths` (plain repo, or a gateway predating the field) skips the step entirely — single-repo behavior bit for bit. Depth-1 only: the gateway parses just the root's `.gitmodules`, so the scoped token never covers nested submodules.
...
Unit: `.gitmodules` parsing (absolute/relative/ssh URLs, non-GitHub hosts skipped), token-scope computation, legacy `prUrl`/`pr_url` reads, prompt with/without the layout section, per-PR feedback grouping, `submodule_paths` excluding out-of-installation submodules. Webhook-level (gateway harness): `pr_urls` callback → multi-link response activity; `completed` on a session with `skippedSubmodules` names them in the response. E2E, both required: (a) **single sandbox repo** — full M1–M4 flow, verifying zero behavior change; (b) **sandbox meta repo with two child submodules** — a cross-repo task yields two child PRs and no pointer-bump commit, and a follow-up updates both; (c) **partial coverage** — a meta repo whose `.gitmodules` includes at least one submodule outside the App installation still completes, PRs only the covered repos, and names the skipped one in the Linear response.
